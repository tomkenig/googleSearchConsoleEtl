...
try:
    ...
    flow = InstalledAppFlow.from_client_secrets_file("client_secret_122350740012-tq24bkpfc93g6c2oc3qrbc2u3ka4mj2f.apps.googleusercontent.com.json", scopes=OAUTH_SCOPE)
...
conn = pyodbc.connect('DRIVER={SQL Server};SERVER=SERVER_NAME;DATABASE=DB_NAME;UID=USER_NAME;PWD=PASS')
    # Using a DSN, but providing a password as well
    #conn = pyodbc.connect('DSN=test;PWD=password') #### OTHER METHOD. All data are stored in odbc config
...
# tomkenig: check function
#print(out_gsc_data('https://domain/', date.strftime("%Y-%m-%d")))
...
def out_pandas_dataframe(argResponse):
    ...
    return df
# test
#print( out_pandas_dataframe(out_gsc_data('https://domain.org/', date.strftime("%Y-%m-%d"))))

# tomkenig: Output as text file
def out_text_file(argDataframe, argPath):
    argDataframe.to_csv(argPath)
# test
#out_text_file(out_pandas_dataframe(out_gsc_data('https://domain.org/', date.strftime("%Y-%m-%d"))), 'gsc_output.csv')
